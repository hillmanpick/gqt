...
description: Review and improve GQT Trader event-contract virtual predictions. Use when the user asks about Binance event contract prediction, 30m/1h virtual orders, prediction win rate, event prediction data collection, backtesting review, or tuning the local event prediction model.
---
...
3. Inspect only `BTCUSDT` and `ETHUSDT`; ignore older records for unsupported symbols and the retired `10m` horizon unless the user explicitly asks for historical 10m analysis. Inspect active horizon-level performance separately for `30m` and `60m`. Do not blend them into one headline number.
...
- The current event prediction bankroll is unlimited virtual capital and each virtual ticket stakes 5 USDT. Losing tickets return 0 and realize `-stake`. Active `30m` and `60m` tickets return principal plus 85% profit (`+4.25` on 5 USDT stake). Retired `10m` tickets may still appear in old SQLite data and used principal plus 80% profit (`+4.00` on 5 USDT stake); do not include them in active-strategy headline stats unless requested. Open exposure is tracked for visibility but must not block new virtual tickets.
