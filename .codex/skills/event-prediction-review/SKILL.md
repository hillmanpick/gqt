...
3. Inspect only `BTCUSDT` and `ETHUSDT`; ignore older records for unsupported symbols. Inspect active horizon-level performance separately for `10m`, `30m`, and `60m`. Do not blend them into one headline number.
